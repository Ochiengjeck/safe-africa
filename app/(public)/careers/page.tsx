...
export default async function CareersPage() {
  ...
  return (
    <main>
      ...
      />
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-14 sm:py-16 lg:grid-cols-[1fr_320px]">
          ...
          <section>
            ...
            {openings.length === 0 ? (
              <div className="mt-5 rounded-xl border border-dashed p-10 text-center">
                ...
                <p className="mt-2 text-muted-foreground">Join our talent pool and we'll notify you when a matching role opens.</p>
              </div>
            ) : (
        ...
        {/* Right column: talent pool, docked */}
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="overflow-hidden rounded-2xl bg-sidebar p-6 text-sidebar-foreground shadow-sm">
            <div className="flex items-center gap-2 text-brand-gold">
              <Users className="h-5 w-5" aria-hidden="true" />
              <span className="font-mono text-xs uppercase tracking-widest">Talent pool</span>
            </div>
            <h2 className="font-display mt-2 text-xl font-bold">No matching role right now?</h2>
            <p className="mt-2 text-sm opacity-85">
              Register your profile and we'll notify you the moment a new opening matches your expertise — and you can be
              invited to interview directly.
            </p>
            <Link
              href="/careers/talent-pool"
              className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-md bg-brand-orange-deep px-6 py-3 font-medium text-white shadow-md transition-opacity hover:opacity-90"
            >
