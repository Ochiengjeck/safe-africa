...
- **Data flow**: Server Components query Prisma directly; all writes are server actions in `lib/actions/*` following one pattern: `requireRole()` → `parseForm(schema, formData)` (Zod, in `lib/validation.ts`) → Prisma → `revalidatePublic()` (revalidates the whole public tree — intentional, the site is small). Admin forms are `useActionState` + the `Field`/`FormError` helpers in `components/admin/field.tsx`; successful saves redirect with `?saved=<message>` which `SavedToast` (mounted in the panel layout) turns into a toast.
- **Soft delete**: Project, Resource, Vacancy, Post, GalleryImage, TeamMember, ContactMessage have `deletedAt` — delete actions set it, `/admin/trash` restores or (SUPER_ADMIN) purges via `lib/actions/trash.ts`. EVERY query on these models must filter `deletedAt: null` (public pages, admin lists, counts, includes) except slug-uniqueness checks, which stay unfiltered on purpose.
- **Admin list pages**: server page fetches rows → client `*-table.tsx` defines columns and renders the shared `components/admin/data-table.tsx` (client-side search/sort/pagination persisted in URL params, mobile card fallback, optional bulk selection — see messages). Destructive actions go through `components/admin/delete-button.tsx` (names the resource, undo toast) — never `window.confirm`. Role-gated pages render `components/admin/access-denied.tsx`, not a redirect.
